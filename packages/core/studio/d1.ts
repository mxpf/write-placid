...
export function createD1Store(options: D1StoreOptions) {
  ...
  async function initializeSchema() {
    ...
    for (const [name, definition] of [["google_doc_id", "TEXT NOT NULL DEFAULT ''"], ["drive_revision", "TEXT NOT NULL DEFAULT ''"], ["drive_synced_body", "TEXT NOT NULL DEFAULT ''"], ["editorial_json", "TEXT NOT NULL DEFAULT '{}'"], ["public_updated_at", "TEXT NOT NULL DEFAULT ''"]] as const) {
      if (!names.has(name)) await db.prepare(`ALTER TABLE documents ADD COLUMN ${name} ${definition}`).run();
    }
  ...
  async function listDocuments() { await ensureSchema(); const rows = await getD1().prepare(`SELECT ${selectColumns} FROM documents ORDER BY type ASC,sort_order ASC,date DESC`).all<Row>(); return (rows.results || []).map(fromRow); }
  async function findDocument(id: string) { await ensureSchema(); const row = await getD1().prepare(`SELECT ${selectColumns} FROM documents WHERE id=? LIMIT 1`).bind(id).first<Row>(); if (row) return fromRow(row); const aliases = (await listDocuments()).filter((document) => document.legacyIds?.includes(id)); if (aliases.length > 1) throw new Error(`Legacy identity collision: ${id}`); return aliases[0]; }
  async function findDocumentByPath(path: string) { await ensureSchema(); const row = await getD1().prepare(`SELECT ${selectColumns} FROM documents WHERE path=? LIMIT 1`).bind(path).first<Row>(); return row ? fromRow(row) : undefined; }
  async function saveDocument(document: WritingDocument, sortOrder = 0) { await ensureSchema(); await getD1().prepare(insertSql).bind(...values(document, sortOrder)).run(); return document; }
  async function cacheDocuments(items: WritingDocument[]) { if (!items.length) return; await ensureSchema(); await getD1().batch(items.map((document) => getD1().prepare(insertSql).bind(...values(document)))); }
  async function replaceDocument(previousId: string, document: WritingDocument, sortOrder = 0) { await ensureSchema(); const current = await getD1().prepare("SELECT sort_order FROM documents WHERE id=? LIMIT 1").bind(previousId).first<{sort_order:number}>(); await getD1().batch([getD1().prepare("DELETE FROM documents WHERE id=?").bind(previousId), getD1().prepare(insertSql).bind(...values(document, current?.sort_order ?? sortOrder))]); return document; }
  async function reorderDraftDocuments(ids: string[]) { await ensureSchema(); const rows = await getD1().prepare("SELECT id,type,status FROM documents").all<{id:string;type:string;status:string}>(); const draftIds = (rows.results || []).filter((row) => row.type === "post" && row.status === "draft").map((row) => row.id); const unique = new Set(ids); if (unique.size !== ids.length || ids.length !== draftIds.length || draftIds.some((id) => !unique.has(id))) throw new Error("The draft list changed. Reload Studio and try again."); if (ids.length) await getD1().batch(ids.map((id, order) => getD1().prepare("UPDATE documents SET sort_order=? WHERE id=?").bind(order, id))); }
  async function seedDocuments(items: WritingDocument[]) { await ensureSchema(); for (const [index, document] of items.entries()) { const existing = await findDocument(document.id); if (!existing) await saveDocument(document, index); } }
  async function deleteDocument(id: string) { const document = await findDocument(id); if (!document) return; const deletedAt = now(); await getD1().batch([getD1().prepare("INSERT INTO deleted_documents (id,document_json,deleted_at) VALUES (?,?,?)").bind(`${deletedAt}:${document.id}`, JSON.stringify(document), deletedAt), getD1().prepare("DELETE FROM documents WHERE id=?").bind(document.id)]); return document; }
  async function getSyncCursor(key: string) { await ensureSchema(); const result = await getD1().prepare("SELECT value FROM sync_state WHERE key=?").bind(key).first<{value:string}>(); const cursor = Number.parseInt(result?.value || "0", 10); return Number.isSafeInteger(cursor) && cursor >= 0 ? cursor : 0; }
  async function setSyncCursor(key: string, cursor: number) { await ensureSchema(); await getD1().prepare("INSERT INTO sync_state (key,value) VALUES (?,?) ON CONFLICT(key) DO UPDATE SET value=excluded.value").bind(key, String(cursor)).run(); }
