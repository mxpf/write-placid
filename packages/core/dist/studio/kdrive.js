import { parseWritingDocument, serializeWritingDocument, } from "./content.js";
import { imageContentType, validImageName } from "./image-files.js";
...
function requestUrl(pathname) {
    ...
    if (!config)
        throw new Error("Write Placid KDrive is not configured.");
...
async function kdriveFetch(pathname, init, allowNotFound = false) {
...
async function listFolder(root, folder = root.folder) {
    const config = configuration();
    if (!config)
        return [];
    ...
    if (!/<(?:[^:>]+:)?multistatus\b/i.test(xml))
        throw new Error("KDrive did not return a complete directory inventory.");
    const entries = [];
    for (const block of xml.match(/<(?:[^:>]+:)?response\b[\s\S]*?<\/(?:[^:>]+:)?response>/gi) || []) {
        const href = block.match(/<(?:[^:>]+:)?href>([\s\S]*?)<\/(?:[^:>]+:)?href>/i)?.[1];
        if (!href)
            throw new Error("KDrive returned an entry without a path.");
        const pathname = decodeURIComponent(new URL(decodeXml(href), config.url).pathname).replace(/\/+$/, "");
        const parent = `${config.root}/${folder}`;
        if (pathname === parent)
            continue;
        if (!pathname.startsWith(`${parent}/`))
            throw new Error("KDrive returned a path outside the requested folder.");
        const name = pathname.slice(parent.length + 1);
        if (name.includes("/"))
            throw new Error("KDrive returned an unexpected nested entry.");
        ...
        if (/<(?:[^:>]+:)?collection\s*\/?\s*>/i.test(block)) {
            ...
            if (!etag || decodeXml(etag).startsWith("W/"))
                throw new Error(`KDrive must provide a strong revision for ${folder}/${name}.`);
            entries.push({ ...root, folder, name, etag: decodeXml(etag) });
        }
    }
    return entries;
}
function plainDraft(source, name, etag) {
...
export async function readKdriveSource(ref) {
    const config = configuration();
    if (!config)
        throw new Error("Write Placid KDrive is not configured.");
...
export async function loadKdrivePost(ref) {
...
export function editorialLocation(document) {
    ...
    if (document.kdrivePath) {
        ...
        if (!oldRoot || oldRoot.type !== document.type)
            throw new Error("Invalid editorial folder for document type.");
        return safeRelativePath(`${root.folder}/${document.kdrivePath.slice(oldRoot.folder.length + 1)}`);
    }
    return `${root.folder}/${document.slug}.md`;
}
async function ensureEditorialParent(relativePath) {
    const config = configuration();
    ...
    for (let index = 1; index <= parts.length; index++) {
        ...
        if (![201, 405].includes(response.status))
            throw new Error(`Could not prepare editorial folder (${response.status}).`);
    }
}
export async function saveKdrivePost(document, previous) {
    const config = configuration();
    if (!config)
        throw new Error("KDrive must be connected before saving editorial content.");
    const destination = editorialLocation(document);
    const prior = previous?.kdrivePath;
    if (previous && prior && !previous.kdriveEtag)
        throw new Error("Reload this piece before saving: its KDrive revision is missing.");
    ...
    if (prior && prior !== destination) {
        ...
        if (await moved.text() !== originalSource)
            throw new Error("The moved file changed in KDrive. Reload before saving.");
        revision = moved.headers.get("ETag") || "";
        if (!revision)
            throw new Error("The folder move succeeded. Reload before editing because KDrive omitted its revision.");
    }
    ...
    if (!document.kdriveEtag) {
        ...
        if (await updated.text() !== serializeWritingDocument(document))
            throw new Error("The saved file changed in KDrive. Reload before editing.");
        document.kdriveEtag = updated.headers.get("ETag") || "";
    }
}
export async function deleteKdrivePost(document) {
    const config = configuration();
    if (!config || !document.kdrivePath || !document.kdriveEtag)
        throw new Error("Reload this piece before deleting it from KDrive.");
...
export async function saveKdriveImage(name, bytes, contentType) {
    const config = configuration();
    if (!config || !validImageName(name))
...
export async function loadKdriveImage(name) {
    const config = configuration();
    if (!config || !validImageName(name))
        return null;
...
//# sourceMappingURL=kdrive.js.map