...
import { CaptionEditor } from "./CaptionEditor.tsx";
import { createEditorRecovery } from "./editor-recovery.ts";
import { displayDate, type WritingDocument } from "./content.ts";
import {
  editorToMarkdown,
...
import { smartenQuotes, smartQuoteForInput } from "./smart-quotes.ts";
import { moveItemToTarget } from "./reorder.ts";
import { articleImageMarkdown } from "./article-images.ts";
import { studioConfig } from "./studio-config.ts";
            ...
            >
              ...
              <small>
                ...
                {document.type === "post" && document.publicUpdatedAt
                  ...
                  : ""}
                {document.kdrivePath ? ` · ${document.kdrivePath.split("/").slice(0, -1).join("/")}` : ""}
...
export function Studio() {
  ...
  const [imageMode, setImageMode] = useState<"add" | "edit">("add");
  ...
  const imageDialogRef = useRef<HTMLElement>(null);
  const imageAltRef = useRef<HTMLInputElement>(null);
  const currentRef = useRef<StudioDocument | null>(null);
  const lastSavedRef = useRef("");
  const saveQueueRef = useRef<(() => Promise<StudioDocument | null>) | null>(null);
  ...
  useEffect(() => {
    if (!imageOpen) return;
    const initialFocus = imageMode === "edit" ? imageAltRef.current : imageDialogRef.current?.querySelector<HTMLInputElement>('input[type="file"]');
  ...
  const selectDocument = useCallback((document: StudioDocument) => {
    let recovered: StudioDocument | null = null;
    try { recovered = recoverEdits(window.sessionStorage, document); } catch { /* Unload warning still protects edits if storage is unavailable. */ }
    ...
    if (recovered) showNotice("Recovered unsaved edits from this tab.");
  ...
  useEffect(() => {
    ...
    fetch("/api/content", { cache: "no-store" })
      ...
      .then((items) => {
        ...
        let recovery: StudioDocument | null = null;
        try { recovery = lastRecoveredDocument(window.sessionStorage); } catch { /* Storage can be disabled. */ }
        const initial = requested || items.find((item) => item.id === recovery?.id) || recovery || items[0];
        setDocuments(items);
        setSaveState("Saved online");
        if (initial) {
          let recovered: StudioDocument | null = null;
          try { recovered = recoverEdits(window.sessionStorage, initial); } catch { /* Storage can be disabled. */ }
  ...
  const persistCurrent = useCallback(async () => {
    ...
    saveQueueRef.current ||= createSaveQueue<StudioDocument>({
    ...
    acknowledge: (saving, saved) => {
      ...
      if (latest && latest.id === saving.id) {
        ...
        try {
          ...
          if (unchanged) forgetEdits(window.sessionStorage, saved.id);
          else rememberEdits(window.sessionStorage, acknowledged);
  ...
  const updateCurrent = useCallback((patch: Partial<StudioDocument>) => {
    setCurrent((document) => {
      ...
      try { rememberEdits(window.sessionStorage, next); } catch { /* Unload warning remains available. */ }
      return next;
    });
  }, []);

  const beginNewPost = async () => {
    if (currentRef.current && !(await persistCurrent())) return;
  ...
  const publishSavedDocument = async (document: StudioDocument) => {
    ...
    try {
      ...
      try {
        if (changedWhilePublishing) rememberEdits(window.sessionStorage, acknowledged);
        else forgetEdits(window.sessionStorage, document.id);
    ...
    for (let attempt = 0; attempt < 18; attempt += 1) {
      ...
      try {
        ...
        if (response.ok && result.live) {
          if (snapshot(currentRef.current) === lastSavedRef.current) setSaveState("Saved online");
  ...
  const publishCurrent = async () => {
    ...
    if (!document || publishingRef.current) return;
    ...
    if (isDraft) {
      ...
      if (!saved) {
        ...
        try { rememberEdits(window.sessionStorage, restored); } catch { /* Keep edits in memory. */ }
        return;
      }
      document = saved;
    }
  ...
  const moveToDraft = async () => {
    ...
    const restored: StudioDocument = { ...previous, kdrivePath: currentRef.current?.kdrivePath, kdriveEtag: currentRef.current?.kdriveEtag, isDirty: true };
    setCurrent(restored);
    currentRef.current = restored;
  ...
  const openImage = (figure: HTMLElement | null = null, trigger: HTMLElement | null = figure) => {
    const editor = bodyRef.current;
    if (!current || !editor) return;
    const existing = figure && editor.contains(figure) ? readEditorImage(figure) : null;
    if (figure && !existing) return;
    const selection = window.getSelection();
    const range = selection?.rangeCount ? selection.getRangeAt(0) : null;
  ...
  const closeImageDialog = () => {
    if (!imageUploading) finishImageDialog();
  };
  ...
  const saveImage = async () => {
    ...
    try {
      ...
      if (!response.ok || !result.src) throw new Error(result.error || "The image could not be added.");
      ...
      if (!imageBlock) throw new Error("The image could not be placed in the editor.");
  ...
  const handleEditorClick = (event: React.MouseEvent<HTMLDivElement>) => {
    ...
    const figure = element?.closest(".editor-image-block")?.querySelector<HTMLElement>("figure.article-image") || element?.closest<HTMLElement>("figure.article-image");
    if (figure) {
      event.preventDefault();
  ...
  const handleImageDialogKeyDown = (event: React.KeyboardEvent<HTMLElement>) => {
    if (event.key === "Escape") {
      event.preventDefault();
      closeImageDialog();
      return;
    }
    if (event.key !== "Tab" || !imageDialogRef.current) return;
    const controls = Array.from(imageDialogRef.current.querySelectorAll<HTMLElement>('input:not(:disabled), button:not(:disabled), [contenteditable="true"]'));
    const first = controls[0];
  ...
  return (
    <main className="studio-shell">
            ...
            <div className="editor">
              ...
              {current.type === "post" && !current.id.startsWith("new:") ? (
                <details className="editorial-details">
                  <summary>Public address and folder</summary>
                  <label htmlFor="slug-input">Public slug</label>
                  <input id="slug-input" key={`${current.id}:${current.slug}`} defaultValue={current.slug} onBlur={(event) => { if (event.target.value !== current.slug) updateCurrent({ slug: event.target.value }); }} />
                  <p>{new URL(`${current.slug}.html`, `${studioConfig.siteUrl.replace(/\/$/, "")}/`).href}</p>
                  <p>Changing the title or folder keeps this address. A new slug keeps the previous address as a redirect.</p>
                  <p>Folder: {current.kdrivePath?.split("/").slice(0, -1).join("/") || "Drafts"}</p>
                  {current.aliases?.length ? <p>Former addresses: {current.aliases.join(", ")}</p> : null}
                </details>
              ) : null}
              ...
              <div className="formatting-toolbar" aria-label="Text formatting">
                ...
                {current.type === "post" ? (
                  <button
                    ...
                    title="Add image"
                  >
                    <ImageIcon aria-hidden="true" size={17} strokeWidth={1.75} />
                  </button>
                ) : null}
        ...
        <div className="dialog-backdrop">
          ...
          <section ref={linkDialogRef} className="link-dialog" role="dialog" aria-modal="true" aria-labelledby="link-title">
            ...
            <label>
              ...
              <label htmlFor="internal-link">Or link to a {studioConfig.publicationName} piece</label>
              <select id="internal-link" value="" onChange={(event) => {
                const target = documents.find((item) => item.id === event.target.value);
                if (target) { setLinkHref(`doc:${encodeURIComponent(target.id)}`); if (!linkLabel) setLinkLabel(target.title); }
              }}>
                <option value="">Choose a piece</option>
                {documents.filter((item) => item.id !== current?.id).map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}
              </select>
      ...
      {imageOpen ? (
        <div className="dialog-backdrop">
          <button className="dialog-dismiss" type="button" aria-label="Close image dialog" onClick={closeImageDialog} />
          {/* The modal owns Escape handling and the focus trap for all descendants. */}
          {/* eslint-disable-next-line jsx-a11y/no-noninteractive-element-interactions */}
          <section ref={imageDialogRef} className="link-dialog image-dialog" role="dialog" aria-modal="true" aria-labelledby="image-title" onKeyDown={handleImageDialogKeyDown}>
            <h2 id="image-title">{imageMode === "edit" ? "Edit image" : "Add an image"}</h2>
            {imageMode === "add" ? <label>
            ...
            <label>
              ...
              <input ref={imageAltRef} value={imageAlt} onChange={(event) => setImageAlt(event.target.value)} placeholder="What the image shows" disabled={imageUploading} aria-describedby="image-description-help" />
            </label>
            <p id="image-description-help">Describe the image for people using screen readers.</p>
            <CaptionEditor value={imageCaption} onChange={setImageCaption} disabled={imageUploading} />
            {imageMode === "add" ? <p>JPEG, PNG, WebP, or GIF · 8 MB maximum</p> : null}
            <div className="dialog-actions">
              <button type="button" onClick={closeImageDialog} disabled={imageUploading}>Cancel</button>
              <button type="button" onClick={saveImage} disabled={imageUploading}>{imageUploading ? "Adding…" : imageMode === "edit" ? "Save image" : "Add image"}</button>
            </div>
          </section>
        </div>
      ) : null}
