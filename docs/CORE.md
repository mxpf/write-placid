...
The package is `@mxpf/write-placid-core`. Version `1.3.0` implements public snapshot contract `1`; both values are exported from the package root. `npm run pack:core` produces a self-contained artifact with ESM JavaScript, declarations, CSS, and source maps. It requires no build or sibling checkout when installed.
