...
export function createD1Store(options: D1StoreOptions) {
  ...
  async function replaceDocument(previousId: string, document: WritingDocument, sortOrder = 0) { await ensureSchema(); const current = await getD1().prepare("SELECT sort_order FROM documents WHERE id=? LIMIT 1").bind(previousId).first<{sort_order:number}>(); await getD1().batch([getD1().prepare("DELETE FROM documents WHERE id=?").bind(previousId), getD1().prepare(replaceSql).bind(...values(document, current?.sort_order ?? sortOrder))]); return document; }
  ...
  async function seedDocuments(items: WritingDocument[]) { await ensureSchema(); if (items.length) await getD1().batch(items.map((document, index) => getD1().prepare(insertDoNothingSql).bind(...values(document, index)))); }
