...
export function smartenQuotes(value) {
    const destinations = [];
    const protectedValue = value.replace(markdownDestinationPattern, (match) => {
        destinations.push(match);
        return `\uE000${destinations.length - 1}\uE001`;
    });
    return smartenProse(protectedValue).replace(/\uE000(\d+)\uE001/g, (_, index) => destinations[Number(index)] ?? "");
}
...
//# sourceMappingURL=smart-quotes.js.map