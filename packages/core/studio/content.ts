...
function parseFrontmatter(source: string) {
  ...
  for (const line of match[1].split("\n")) {
    ...
    if (separator === -1 || /^\s/.test(line)) throw new Error("Frontmatter must use one field per line; preserve and convert this file before editing.");
    const key = line.slice(0, separator).trim();
    if (Object.hasOwn(metadata, key)) throw new Error(`Duplicate frontmatter field: ${key}`);
...
export function snapshotPath(type: WritingDocument["type"], publicPath: string) {
  if (!/^[a-z0-9][a-z0-9._-]*\.md$/.test(publicPath)) throw new Error(`Invalid public filename: ${publicPath}`);
...
export function normalizeIncomingDocument(
  ...
  if (existing && input.id && input.id !== existing.id) throw new Error("Document IDs cannot change.");
  ...
  if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(slug)) throw new Error("Use a public slug with lowercase letters, numbers, and hyphens.");
...
export function isDocumentDirty(document: WritingDocument) {
  if (!document.publishedSource.trim()) return true;
